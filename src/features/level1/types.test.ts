...
import { isGrid, isLevel1State, isAnswerResult, isPuzzleList } from './types.ts';
const state = { status: 'FIND_PARTNER', name: 'Name', participant_code: 'CODE', fragment_slot: 'A', assigned_grid: [['a', '']], mutual_verified: false, solved: false };
...
test('safe states preserve the assigned grid before and after solving', () => {
  ...
  assert.ok(isLevel1State({ status: 'NOT_PAIRED', name: 'Name', participant_code: 'CODE' }));
  assert.ok(isLevel1State({ status: 'NO_PUZZLE', name: 'Name', participant_code: 'CODE', fragment_slot: 'A', mutual_verified: false, solved: false }));
});
